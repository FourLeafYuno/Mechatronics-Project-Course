    ...
    for item in cnt_list:
        if item[2] == y_list[-1]:
            ...
            if Vis:
                ...
                cv2.line(img,( int(640/2),480), (item[1], item[2]), (0,255,0),1)
...
while True:
    ...
    points = polygon(frameHeight,frameWidth,Sens)
    #print(points)
    imgImp = superimpose(img, points) 
    ...
    contours,hierarchy  = poly_cont(imgBlur)
    #contours = getContours(imgDil,imgContours)
    findpath(img,imgContours,points,contours, True)
    imgStack = stackImages(0.8,([img,imgContours],[img,img]))
